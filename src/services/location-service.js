/* ==========================================================================
   AEGIS-MESH / RESQNET — CENTRALIZED LIVE USER LOCATION SERVICE
   ...
   distance calculations, and cross-context synchronization.
   ========================================================================== */
...
class LocationService {
  ...
  startWatchingGPS() {
    ...
    if (typeof navigator === 'undefined' || !navigator.geolocation) {
      console.warn('[LocationService] Browser does not support geolocation.');
    ...
    const errorHandler = (error) => {
      ...
      // Seamless fallback to demo disaster location
      this.state = {
        ...this.state,
        latitude: DEMO_USER_LOCATION.latitude,
        longitude: DEMO_USER_LOCATION.longitude,
  ...
  /* --------------------------------------------------------------------------
     DEMO LOCATION & SIMULATED EVACUATION MOVEMENT
     -------------------------------------------------------------------------- */
  setDemoLocation() {
  ...
  startMovementSimulation() {
    ...
    // Bounded realistic evacuation route in Sector B4:
    const waypoints = [
      [20.2961, 85.8245], // Rooftop Sector B4
      [20.2945, 85.8225], // Sector B4 Flooded Alley
      [20.2925, 85.8198], // Elevated Flyover Ramp
      [20.2895, 85.8170], // Janpath Road High Ridge
      [20.2865, 85.8140], // St. Jude Shelter Corridor
      [20.2850, 85.8120], // St. Jude Relief Hub Safe Zone
      [20.2880, 85.8160], // Patrol Return Loop
      [20.2930, 85.8210], // Near Rescue Boat Alpha
      [20.2961, 85.8245]  // Return to origin
    ];

    let currentWpIdx = 0;
    let step = 0;
    const totalSteps = 30; // 30 steps between waypoints
    ...
    this.simulationTimer = setInterval(() => {
      ...
      this.state = {
        ...
        speed: 14, // 14 km/h evacuation speed
        timestamp: Date.now(),
        source: 'SIMULATED',
        status: 'ACTIVE',
        tracking: true
  ...
  /* --------------------------------------------------------------------------
     GEOGRAPHIC & DISTANCE COMPUTATION UTILITIES
     -------------------------------------------------------------------------- */
  ...
  getComprehensiveInspectorData() {
    const state = this.getState();
    const nearestHospital = this.getNearestHospital();
    const nearestShelter = this.getNearestShelter();
    const nearestRescue = this.getNearestRescueTeam();

    const elapsedSeconds = Math.max(0, Math.round((Date.now() - state.timestamp) / 1000));
    let updatedText = 'Just now';
    if (elapsedSeconds > 1 && elapsedSeconds < 60) updatedText = `${elapsedSeconds}s ago`;
    else if (elapsedSeconds >= 60) updatedText = `${Math.floor(elapsedSeconds / 60)}m ago`;
...
// Expose globally for vanilla scripts / cross-framework compatibility
if (typeof window !== 'undefined') {
